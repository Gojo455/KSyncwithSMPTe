...
def compute_seat_quality(row, col, total_rows, total_cols):
    ...
    Derived from SMPTE EG-18-1994 viewing angle thresholds:

    ROW SCORE:
      The SMPTE optimal horizontal viewing angle (40–50 degrees) corresponds
      to a seating zone approximately 40–65% back from the screen.
      The midpoint of this zone (52.5% back) is used as the optimal row.
      Score decays linearly from 1.0 at the optimum toward the boundaries:
        - Below 20% (SMPTE 35-degree vertical discomfort threshold): heavy penalty
        - Above 85% (below SMPTE 30-degree minimum horizontal angle): moderate penalty

    COLUMN SCORE:
      SMPTE and ITU-R BT.2022 both constrain lateral viewing angle to ≤ 30 degrees.
      Dead centre (50%) satisfies this for all standard hall widths.
      Score decays linearly from 1.0 at centre toward the edges.
      Outer 20% on either side falls outside the acceptable lateral angle range.

    Final score: row contributes 60%, column 40% — consistent with SMPTE's
    emphasis that longitudinal distance from screen has greater perceptual
    impact than lateral offset.
    """
